...
class KMeans:
    ...
    def __int__(self, k, max_iter = 300, distance = euclidean_distance):
    ...
    def fit(self, dataset:Dataset):
        ...
        seeds = np.permutation(dataset.X.shape[0][:self.k])
        ...
        while convergence and cont < self.max_iter:
            new_labels = np.apply_along_axis(self._closest_centroid, axis=1, arr=dataset.X, centroid=self.centroids) #Linhas
            all_centroids =[]
